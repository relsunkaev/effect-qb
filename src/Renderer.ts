...
export interface Renderer<Dialect extends string = string> {
  readonly dialect: Dialect
  render<PlanValue extends Query.QueryPlan<any, any, any, any, any, any, any>>(
    plan: Query.DialectCompatiblePlan<PlanValue, Dialect>
  ): RenderedQuery<Query.ResultRow<PlanValue>, Dialect>
}

type CustomRender<Dialect extends string> = <PlanValue extends Query.QueryPlan<any, any, any, any, any, any, any>>(
  plan: Query.DialectCompatiblePlan<PlanValue, Dialect>
) => {
  readonly sql: string
  readonly params?: readonly unknown[]
  ...
  return {
    ...
    render(plan) {
      ...
      return {
        ...
        dialect,
        [TypeId]: {
          row: undefined as Query.ResultRow<typeof plan>,
          dialect
        }
      }
    }
