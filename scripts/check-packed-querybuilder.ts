...
const main = async () => {
  ...
  try {
    ...
    await Bun.write(join(consumerDir, "index.ts"), [
      ...
      "",
      'const standardUsers = Sql.Table.make("standard_users", {',
      "  id: Sql.Column.uuid().pipe(Sql.Column.primaryKey),",
      "  email: Sql.Column.text()",
      "})",
      "",
      "const standardPlan = Sql.Query.select({",
      "  id: standardUsers.id,",
      "  email: Sql.Function.lower(standardUsers.email)",
      "}).pipe(Sql.Query.from(standardUsers))",
      "",
