...
describe("ddl rendering behavior", () => {
  ...
  test("column DDL type overrides trust typed db type names without renderer-time validation", () => {
    ...
    const plan = StdRoot.Query.createTable(assets)
  ...
  test("DDL uses table-level casing for constraints and index names", () => {
    ...
    expect(Mysql.Renderer.make().render(StdRoot.Query.createTable(users)).sql).toContain(
      "constraint `account_status_check` check ((`account_status` = 'active'))"
    )
    expect(Sqlite.Renderer.make().render(StdRoot.Query.createTable(users)).sql).toContain(
      `constraint "account_status_check" check (("account_status" = 'active'))`
    )
    expect(Postgres.Renderer.make().render(StdRoot.Query.createIndex(users, ["accountStatus"])).sql).toBe(
      `create index "users_account_status_idx" on "users" ("account_status")`
    )
    expect(Postgres.Renderer.make().render(StdRoot.Query.dropIndex(users, ["accountStatus"])).sql).toBe(
    ...
    const users = StdRoot.Table.make("UserAccounts", {
      ...
      normalizedName: StdRoot.Column.text().pipe(
        StdRoot.Column.generated(
          StdRoot.Function.coalesce(StdRoot.Query.column("displayName", StdRoot.Type.text()), "")
        )
      )
    }).pipe(
      Casing.withCasing({
  ...
  test("rejects identity columns where unsupported by the dialect", () => {
    ...
    expect(Postgres.Renderer.make().render(StdRoot.Query.createTable(users)).sql).toBe(
    ...
    expect(() =>
      Mysql.Renderer.make().render(StdRoot.Query.createTable(users))
    ).toThrow("Unsupported mysql identity column options")
    expect(() =>
      Sqlite.Renderer.make().render(StdRoot.Query.createTable(users))
  ...
  test("rejects array columns where unsupported by the dialect", () => {
    ...
    expect(Postgres.Renderer.make().render(StdRoot.Query.createTable(docs)).sql).toBe(
    ...
    expect(() =>
      Mysql.Renderer.make().render(StdRoot.Query.createTable(docs))
    ).toThrow("Unsupported mysql array column options")
    expect(() =>
      Sqlite.Renderer.make().render(StdRoot.Query.createTable(docs))
  ...
  test("postgres and mysql DDL expressions inline literals instead of bind parameters", () => {
    ...
    const renderedMysql = Mysql.Renderer.make().render(StdRoot.Query.createTable(mysqlUsers))
  ...
  test("rejects invalid foreign key actions at runtime", () => {
    ...
    expect(() =>
      Postgres.Renderer.make().render(StdRoot.Query.createTable(postgresMemberships))
    ...
    expect(() =>
      Mysql.Renderer.make().render(StdRoot.Query.createTable(mysqlMemberships))
    ).toThrow("Foreign key action must be noAction, restrict, cascade, setNull, or setDefault")
    ...
    expect(() =>
      Sqlite.Renderer.make().render(StdRoot.Query.createTable(sqliteMemberships))
    ).toThrow("Foreign key action must be noAction, restrict, cascade, setNull, or setDefault")
  })
  ...
  test("mysql and sqlite DDL casing maps physical identifiers", () => {
    ...
    expect(Mysql.Renderer.make().render(StdRoot.Query.createIndex(memberships, ["createdAt"])).sql).toBe(
      "create index `membership_records_created_at_idx` on `membership_records` (`created_at`)"
    )
    expect(Mysql.Renderer.make().render(StdRoot.Query.dropIndex(memberships, ["createdAt"])).sql).toBe(
      "drop index `membership_records_created_at_idx` on `membership_records`"
    )
    expect(Sqlite.Renderer.make().render(StdRoot.Query.createTable(memberships)).sql).toContain(
    ...
    expect(Sqlite.Renderer.make().render(StdRoot.Query.createIndex(memberships, ["createdAt"])).sql).toBe(
      'create index "membership_records_created_at_idx" on "membership_records" ("created_at")'
    )
    expect(Sqlite.Renderer.make().render(StdRoot.Query.dropIndex(memberships, ["createdAt"])).sql).toBe(
  ...
  test("rejects unknown table option kinds at render time", () => {
    ...
    expect(() =>
      Postgres.Renderer.make().render(StdRoot.Query.createTable(postgresUsers))
    ...
    expect(() =>
      Mysql.Renderer.make().render(StdRoot.Query.createTable(mysqlUsers))
    ).toThrow("Unsupported table option kind")
    ...
    expect(() =>
      Sqlite.Renderer.make().render(StdRoot.Query.createTable(sqliteUsers))
    ).toThrow("Unsupported table option kind")
  })
  ...
  test("ignores malformed table option entries without runtime validation", () => {
    ...
    expect(Postgres.Renderer.make().render(StdRoot.Query.createTable(postgresUsers)).sql).toContain(
      'create table "users" ("id" uuid not null, primary key ("id"))'
    )
    expect(Mysql.Renderer.make().render(StdRoot.Query.createTable(mysqlUsers)).sql).toContain(
      "create table `users` (`id` char(36) not null, primary key (`id`))"
    )
    expect(Sqlite.Renderer.make().render(StdRoot.Query.createTable(sqliteUsers)).sql).toContain(
  ...
  test("rejects malformed foreign key reference columns before rendering DDL", () => {
    ...
    expect(() =>
      Postgres.Renderer.make().render(StdRoot.Query.createTable(users))
    ).toThrow()
  })
  ...
  test("rejects malformed check constraints before rendering DDL", () => {
    ...
    ;(mysqlUsers as any)[StdRoot.Table.OptionsSymbol] = [{
      kind: "check",
      name: 123,
      predicate: StdRoot.Query.eq(mysqlUsers.id, "user-id")
    }]
  ...
  test("ddl builders trust typed clause kinds without renderer-time validation", () => {
    ...
    const postgresCreate = StdRoot.Query.createTable(postgresUsers, { ifNotExists: true })
    ...
    const mysqlCreate = StdRoot.Query.createTable(mysqlUsers, { ifNotExists: true })
    ...
    const sqliteCreate = StdRoot.Query.createTable(sqliteUsers, { ifNotExists: true })
  ...
  test("rejects non-portable constraint options where unsupported by the dialect", () => {
    ...
    expect(() =>
      Mysql.Renderer.make().render(StdRoot.Query.createTable(memberships))
  ...
  test("postgres drop index qualifies indexes for schema-scoped tables", () => {
    ...
    const rendered = Postgres.Renderer.make().render(StdRoot.Query.dropIndex(events, ["userId"], {
      ifExists: true
    }))
